...
namespace Seemon.Authenticator.Services
{
    public class EncryptionService : IEncryptionService
    {
        ...
        private const int _minimumEncryptedMessageByteSize =
            ...
            _aesBlockByteSize +     // Cipher text min lenght
            _signatureByteSize;     // signature tag

        public static readonly Encoding _stringEncoding = Encoding.UTF8;

        public string EncryptAndSign(string value, SecureString password)
        {
            //Encrypt
        ...
        public string VerifyAndDecrypt(string encryptedValue, SecureString password)
        {
            var encrypted = Convert.FromBase64String(encryptedValue);
            if(encrypted == null || encrypted.Length < _minimumEncryptedMessageByteSize)
            {
            ...
            using(var hmac = new HMACSHA256(authKey))
            {
                ...
                {
                    throw new CryptographicException("Invaid signature.");
                }
            }
